...
def train_model(policy, baseline, trajs, policy_optim, baseline_optim, device, gamma=0.99, baseline_train_batch_size=64,
    ...
    for epoch in range(baseline_num_epochs):
        ...
        for i in range(n // baseline_train_batch_size):
            ...
            # 2 lines of code
            baseline_pred = baseline(inputs)
            loss = criterion(baseline_pred, target)
    ...
    # loss = -torch.mean(log_probs * (torch.Tensor(returns).to(device) - baseline_pred))
    loss = -torch.mean(log_probs * (torch.Tensor(returns).to(device)))
